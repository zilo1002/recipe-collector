...
export default function FilterSection() {
  const { sortOptions, setSortOptions, customIngredientSubCategories, customIngredientCategories, recipes } = useRecipeStore();
  ...
  const getSubCategories = (parentCategory: IngredientCategory): string[] => {
    // 如果是默认分类，使用默认二级分类 + 该分类下的自定义二级分类
    if (DEFAULT_INGREDIENT_SUB_CATEGORIES[parentCategory]) {
      const defaults = DEFAULT_INGREDIENT_SUB_CATEGORIES[parentCategory] || [];
      // 获取所有菜谱中属于该一级分类的食材名称
      const customSubsForThisCategory = recipes
        .flatMap(r => r.selectedIngredients || [])
        .filter(ing => ing.category === parentCategory && ing.name)
        .map(ing => ing.name.trim());
      return [...new Set([...defaults, ...customSubsForThisCategory])];
    }
    // 如果是自定义一级分类，只显示属于该分类的食材
    const customSubsForThisCategory = recipes
      .flatMap(r => r.selectedIngredients || [])
      .filter(ing => ing.category === parentCategory && ing.name)
      .map(ing => ing.name.trim());
    return [...new Set([...customSubsForThisCategory])];
  };
