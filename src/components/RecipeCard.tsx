...
import { Clock, Leaf, Trash2, Eye, Edit2, Flame, ShoppingBasket } from 'lucide-react';
import {
  type Recipe,
...
export default function RecipeCard({ recipe, onDelete }: RecipeCardProps) {
  ...
  // 从食材中提取二级分类
  const getIngredientSubCategories = (): string[] => {
    if (!recipe.ingredients || recipe.ingredients.length === 0) return [];
    const subCats = recipe.ingredients
      .map(ing => ing.name)
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`h-3 ${
            (COOKING_METHOD_COLORS[recipe.cookingMethod] || 'bg-gray-200').split(' ')[0]
          }`}
        />
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span
              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${
                COOKING_METHOD_COLORS[recipe.cookingMethod] || 'bg-gray-100 text-gray-700'
              }`}
            >
            ...
            {recipe.ingredientCategory && (
              <span
                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${
                  INGREDIENT_CATEGORY_COLORS[recipe.ingredientCategory] || 'bg-gray-100 text-gray-700'
                }`}
              >
            ...
            {/* 显示食材二级分类标签 */}
            {getIngredientSubCategories().map((subCat, idx) => (
              <span
                key={idx}
